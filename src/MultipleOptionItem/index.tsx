...
import { Form, Input, InputProps, Select, SelectProps, FormItemProps } from 'antd';
...
const InputMultipleOption = ({
  ...
  const [placeholder, setPlaceholder] = React.useState(`请输入${options?.[0].name || '...'}`);
  return (
    <Form.Item {...rest}>
      <Input.Group 
        compact 
      ...
      >
        <Form.Item noStyle name={optionItemName}>
          <Select
            onChange={(value: string) => {
              setPlaceholder(
                `请输入${options.find((option) => option.value === value)?.name || '...'}`
              );
            }}
            {...selectProps}
            className={cx("tst-multiple-option-Input-form-select", selectProps?.className)}
          >
        ...
        <Form.Item noStyle name={inputItemName} rules={inputRules}>
          <Input
            ...
            style={inputStyle}
            {...inputProps}
            className={cx("tst-multiple-option-Input-form-select-input", inputProps?.className)}
          />
        </Form.Item>
      </Input.Group>
    </Form.Item>

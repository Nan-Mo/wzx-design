...
export const showExportAnimation = async (position: { x: number; y: number }) => {
    ...
    if (targetDom.getBoundingClientRect) {
      ...
      target = {
        x:
          ...
          targetDom.parentElement!.parentElement!.parentElement!.nextElementSibling!.clientWidth -
          24,
        y: 20,
      };
    }
...
export function isSuccResponse(response: IResponse<any>, defaultMsg?: string) {
  ...
  if (response.code !== 200) return response.msg && message.error(response.msg) && false;
  return true;
}
